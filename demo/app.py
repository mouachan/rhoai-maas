...
<div id="login-section" class="container">
  <div class="login-box">
    ...
    <p>Enter your OpenShift token to access the LLM</p>
...
<div id="chat-section" class="container">
  <div class="chat-wrap" id="chat-wrap">
    ...
    <div class="sidebar">
      ...
      <div class="card">
        <div class="bar-wrap">
          <div class="bar-label"><span id="sb-rl-label">Rate Limit (0/5 req/min)</span><span id="sb-rl-pct">0%</span></div>
...
const TIER_LIMITS={free:5,premium:60,enterprise:600};
let session=null;
let history=[];
...
function updateSidebar(){
  ...
  const limit=TIER_LIMITS[session.tier]||10;
  const pct=Math.min(100,(session.requests/Math.max(1,limit))*100);
  const bc=pct<70?'#2ecc71':pct<90?'#f39c12':'#e74c3c';
  document.getElementById('sb-rl-label').textContent='Rate Limit ('+session.requests+'/'+limit+' req/min)';
...
document.getElementById('ocp-token').addEventListener('keydown',function(e){if(e.key==='Enter')doLogin();});
</script>
</body>
...
def api_login():
    ...
    try:
        resp = http_requests.post(
            ...
            headers={"Authorization": f"Bearer {ocp_token}"},
            json={"groups": []}, verify=False, timeout=10,
        )
    ...
    def generate():
        ...
        try:
            ...
            if resp.status_code == 429:
                ...
                yield 'data: ' + json.dumps({"type": "error", "content": f"RATE LIMITED (429) -- {s['tier']} tier limit reached."}) + '\n\n'
